...
客戶管理用於查看個人 KYC、企業 KYB、開戶資料和開戶進度；「投資者」模組則更偏向交易帳戶及資產管理。

### 個人用戶列表

可按姓名、手機號碼、電郵或 VC User ID 搜尋，並按開戶狀態篩選。列表展示用戶資訊、聯繫方式、首選語言、開戶狀態、Sumsub 狀態、資料預覽、註冊時間和詳情入口。
...
| 區塊 | 核對重點 |
| --- | --- |
| **身份與帳戶** | 姓名、聯繫方式、國籍、證件類型、首選語言和開戶狀態 |
| **KYC** | Sumsub 狀態、Applicant ID、Inspection ID、申請平台和 IP 國家 |
| **風險與補充資料** | 風險評估結果、補充問卷及待補項目 |
| **就業與財務** | 職業、收入、資產、資金來源及投資經驗 |
| **合同與費用** | 協議確認、開戶資料和當前費用設定 |
| **系統資訊** | 內部用戶 ID、VC User ID、角色、客戶類型和更新時間 |

![個人客戶詳情](../assets/V102/customers-personal-detail.png)

> ⚠️ 證件影像、聯繫資料、財務數據等屬於敏感資訊，僅應在業務需要範圍內查看和使用。

### 企業用戶列表

企業列表支援按公司名稱、註冊號、聯繫人、電郵、手機號碼或 VC User ID 搜尋，並可按資料或開戶狀態篩選。

![企業客戶列表](../assets/V102/customers-enterprise-list.png)

列表展示公司資訊、聯繫方式、企業類型與註冊地、資料狀態、開戶狀態、建立時間和操作入口。先用公司名稱和註冊號確認記錄，再打開操作選單。
...
2. 頂部步驟欄顯示當前階段：**主體資料 → 財務與結構 → 合規與帳戶 → 稅務與文件 → 文件簽署**；
3. 「儲存草稿」儲存當前已填內容，適合暫時離開；
4. 「儲存並下一步」先驗證當前階段的必填項，再進入下一階段；
5. 使用「上一步」返回時，先儲存當前修改；
6. 重新進入列表後，從同一草稿繼續，先檢查階段、欄位和已上傳文件是否完整。

帶 `*` 的欄位為當前階段必填項。驗證未通過時，應按頁面提示逐項修正，不要用無意義字符繞過驗證。
...
| **聯繫人** | 聯繫人姓名、電郵、區號與電話、辦公室電話、可選傳真、結單電郵和補充資訊 |

![企業開戶第 1 步：主體資料](../assets/V102/customers-enterprise-onboarding-step-1-main.jpg)

### 第 2 步：財務與結構

該階段用於說明企業財務狀況及控制結構。按當前頁面填寫財務概況、資金或財富來源，並逐項登記董事、股東和最終受益人。
...
該階段用於記錄合規聲明、預期帳戶用途和帳戶相關選擇。逐項閱讀問題，根據獲批資料作答，並核對帳戶聯繫人或結算資料。
...
填寫稅務居住地、稅號或頁面要求的稅務聲明，並上傳當前介面列出的企業文件。
...
| **當前階段驗證失敗** | 留在當前階段，按提示補齊必填或修正格式 |
| **待複核 / 已提交** | 按頁面狀態和組織審核流程處理，不要重複建立另一條申請 |
| **需更正** | 先確認當前狀態是否允許返回編輯；不能編輯時交由授權人員處理 |
