...
- **開戶協議**：上傳協議文件，並設定協議識別碼、標題、語言、是否必簽及顯示順序。

> 🚨 錢包鏈類型或地址錯誤可能造成資產永久損失。提交前應複製回查完整地址，並由第二人複核。

協議新增或替換後同樣需要審核。多語言協議應使用一致的協議識別碼，並檢查客戶端的顯示順序和必簽狀態。
...
在港股、美股等標籤中搜尋並新增推薦指數，移除不再顯示的項目並調整順序，最後提交審核。股票推薦設定同樣按市場維護；顯示數量及順序以後台目前的限制為準。
